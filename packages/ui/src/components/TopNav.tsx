...
function HamburgerIcon() {
  // Drawn from three plain Views rather than a glyph/icon font — renders
  // identically on native and web with no font-availability risk.
  return (
    <View style={{ gap: 4 }}>
      {[0, 1, 2].map((i) => (
        <View key={i} style={{ width: 20, height: 2, borderRadius: 1, backgroundColor: colors.textPrimary }} />
      ))}
    </View>
  );
}
...
// NavLink children; this owns layout (including the responsive collapse
// below the `md` breakpoint) only, never routing.
export function TopNav({ brand, children, activeKey }: TopNavProps) {
  ...
  return (
    <View style={{ backgroundColor: colors.surface, borderBottomWidth: 1, borderBottomColor: colors.border }}>
      ...
      >
        ...
        {isCompact ? (
          <Pressable
            ...
            hitSlop={8}
            style={{ padding: spacing[2], borderRadius: radii.sm }}
          >
            <HamburgerIcon />
          </Pressable>
        ) : (
      ...
      {isCompact ? (
        <Modal visible={menuOpen} onClose={() => setMenuOpen(false)} title="Menu">
